...
/// Sort matches in-place by descending score, then directories before files,
/// then ascending path.
#[cfg(test)]
fn sort_matches(matches: &mut [(u32, bool, String)]) {
    matches.sort_by(cmp_by_score_desc_then_directory_then_path_asc::<
        (u32, bool, String),
        _,
        _,
        _,
...
/// items by descending score, then directories before files, and then
/// ascending path using the provided accessors.
pub fn cmp_by_score_desc_then_directory_then_path_asc<T, FScore, FDirectory, FPath>(
    score_of: FScore,
    is_directory_of: FDirectory,
    path_of: FPath,
) -> impl FnMut(&T, &T) -> std::cmp::Ordering
where
    FScore: Fn(&T) -> u32,
    FDirectory: Fn(&T) -> bool,
    FPath: Fn(&T) -> &str,
{
    use std::cmp::Ordering;
    move |a, b| match score_of(b).cmp(&score_of(a)) {
        Ordering::Equal => match is_directory_of(b).cmp(&is_directory_of(a)) {
...
mod tests {
    ...
    #[test]
    fn tie_breakers_prefer_directories_then_sort_by_path_when_scores_equal() {
        let mut matches = vec![
            (100, false, "b_path".to_string()),
            (100, true, "b_path".to_string()),
            (100, true, "a_path".to_string()),
            (90, false, "zzz".to_string()),
        ];

        sort_matches(&mut matches);

        // Highest score first; ties prefer directories and then alphabetical order.
        let expected = vec![
            (100, true, "a_path".to_string()),
            (100, true, "b_path".to_string()),
            (100, false, "b_path".to_string()),
