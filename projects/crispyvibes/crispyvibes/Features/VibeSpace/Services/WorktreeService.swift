...
enum WorktreeParser {
    ...
    static func parse(
        ...
        func flush() {
            ...
            entries.append(WorktreeEntry(path: normalized, branch: currentBranch))
        }
    ...
    func probe(paths: [String]) async -> WorktreeProbeResult {
        ...
        return await Task.detached(priority: .utility) {
            ...
            for path in paths where !path.hasPrefix("ssh://") {
                ...
                let branch = Self.output(run, ["-C", path, "rev-parse", "--abbrev-ref", "HEAD"])
                result.infoByProject[path] = ProjectWorktreeInfo(commonDir: commonDir, branch: branch)
